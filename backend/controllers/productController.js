...
const getProductById = asyncHandler(async (req, res) => {
  // Nota: verificar el ObjectId válido para evitar que CastError se mueva a separar
  // middleware.
  const product = await Product.findById(req.params.id);
  if (product) {
    return res.json(product);
  } else {
    // NOTA: esto se ejecutará si se encontró un ObjectId válido pero no un producto
...
const createProductReview = asyncHandler(async (req, res) => {
  ...
  if (product) {
    ...
    if (alreadyReviewed) {
      res.status(400);
      throw new Error('Producto ya ha si reseñado');
    }
